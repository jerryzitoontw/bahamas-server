...
-----------------------------------------------------------------------------------------------------------------------------------------
Bahamas = {}
Tunnel.bindInterface("routes",Bahamas)
					...
					if IsControlJustPressed(1,38) and inSeconds <= 0 then
						...
						if not inService then
							if vSERVER.checkPermission(k) then
								...
								TriggerEvent("Notify",false,"Serviço iniciado.","amarelo",5000)
							end
						else
							if inService == k then
								lastService = k
								inService = false
								TriggerEvent("Notify",false,"Serviço finalizado.","amarelo",5000)
								...
								TriggerEvent("Notify",false,"Finalize o emprego anterior para iniciar um novo.","amarelo",5000)
							end
						end
					end
				end
							...
							if routes[inService]["routeCollect"] then
								if routes[inService]["collectVehicle"] ~= nil then
									...
									if IsVehicleModel(vehicle,vehHash) then
										...
										TriggerEvent("Notify",false,"Precisa utilizar o veículo do <b>"..inService.."</b>.","amarelo",3000)
									end
								else
									if vSERVER.collectConsume(inService) then
										if inCollect >= #routes[inService]["collectCoords"] then
...
-- THREADSECONDS
-----------------------------------------------------------------------------------------------------------------------------------------
function Bahamas.updateRoutes(status)
	routes = status
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- DRAWTEXT3D
