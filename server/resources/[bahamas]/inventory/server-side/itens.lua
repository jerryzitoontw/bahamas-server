...
Use = {
	...
	["backpack"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if vRP.GetWeight(Passport) >= 100 then
			TriggerClientEvent("Notify",source,"Aviso","Limite de <b>"..itemName("backpack").."</b> atingido.","amarelo",5000)
		    return
		end
		...
		repeat
			if os.time() >= parseInt(Active[Passport]) then
				...
				if vRP.TakeItem(Passport,Full,1,true,Slot) then
					...
					TriggerClientEvent("inventory:Update",source,"Backpack")
					TriggerClientEvent("Notify",source,"Aviso","<b>"..itemName("backpack").."</b> usada.","verde",5000)
				end
			end
		...
		if not vRP.UserPremium(Passport) then
			...
			if vRP.HasPermission(Passport,"Premium",1) and vRP.TakeItem(Passport,Full,1,true,Slot) then
				...
				if vRP.GetWeight(Passport) >= 125 then
					TriggerClientEvent("Notify",source,"Aviso","Sua mochila de <b>25kg</b> não foi adicionada pois você atingiu o limite.","amarelo",5000)
					return
				end
		...
		if vehicle[1] then
			...
			if Keyboard then
				...
				if string.len(plateCheck) ~= 8 then
					TriggerClientEvent("Notify",source,"Aviso","O nome de definição para a placa inválida.","vermelho",5000)
					return
				else
	...
	["chip"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		if Keyboard then
			...
			if string.len(Fir) == 3 and string.len(Sec) == 3 then
				...
				TriggerClientEvent("Notify",source,"Aviso","O número telefônico deve conter 6 dígitos e somente números.","vermelho",5000)
			end
		end
	end,
	...
	["tyres"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not vRPC.InsideVehicle(source) then
			if not vCLIENT.checkWeapon(source,"WEAPON_WRENCH") then
				TriggerClientEvent("Notify",source,"vermelho","<b>Chave Inglesa</b> não encontrada.","Aviso",5000)
				return
			end
